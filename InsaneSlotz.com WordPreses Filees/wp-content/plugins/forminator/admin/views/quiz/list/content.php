...
$search_keyword = null;
$is_search = false;
$search_quizzes_nonce = esc_attr( 'forminator-nonce-search-module' );
if ( isset( $_GET['search'] ) && check_admin_referer( $search_quizzes_nonce, $search_quizzes_nonce ) ) {
...
// Get modules
$quiz_modules 	  = $this->getModules( $search_keyword );
$quiz_modules_all = $this->getModules();
...
// Count total quizzes
$count        = ! $is_search ? $this->countModules() : count( $quiz_modules );
...
$most_entry = Forminator_Form_Entry_Model::get_most_entry( 'quizzes' );
?>
	...
	<div class="sui-box sui-summary sui-summary-sm <?php echo esc_attr( $this->get_box_summary_classes() ); ?>">
		...
		<div class="sui-summary-segment">
			...
			<div class="sui-summary-details">
				...
				<span class="sui-summary-sub"><?php printf( esc_html( _n( 'Active Quiz', 'Active Quizzes', esc_html( $count_active ), Forminator::DOMAIN ) ), esc_html( $count_active ) ); ?></span>

				<form action="" method="get" id="forminator-search-quizzes" class="forminator-search-modules">
						...
						<div class="sui-col-lg-10 sui-col-md-12">
							...
							<div class="sui-form-field">
								...
								<div class="sui-control-with-icon">
									...
									<input type="text" name="search" value="<?php esc_attr_e( $search_keyword ); ?>" placeholder="<?php esc_attr_e( 'Search quiz...', Forminator::DOMAIN ); ?>" id="forminator-quizzes-search" class="sui-form-control">
								</div>
					...
					<input type="hidden" name="page" value="forminator-quiz" />
					<?php
						wp_nonce_field( $search_quizzes_nonce, $search_quizzes_nonce, false );
					?>
			...
			<ul class="sui-list">
				...
				<li>
					...
					<span class="sui-list-detail">
						<?php
							if ( ! empty( $most_entry ) && get_post_status( $most_entry->form_id ) && 0 !== (int) $most_entry->entry_count ) {
								$most_entry_model = Forminator_Quiz_Form_Model::model()->load( $most_entry->form_id );
						?>
							<a href="<?php echo esc_url( admin_url( 'admin.php?page=forminator-' . $most_entry_model->quiz_type . '-wizard&id=' . $most_entry->form_id ) ); ?>">
								<?php echo esc_html( forminator_get_form_name( $most_entry->form_id, 'quiz' ) ); ?>
							</a>
						<?php
							} else {
								echo esc_html( '&mdash;' );
	...
	<div class="sui-box sui-message">
		...
		<div class="sui-message-content">
			...
			<?php else: // Search no-result message ?>
				...
				<p><?php esc_html_e( "We couldn't find any quizzes matching your search keyword. Perhaps try again?", Forminator::DOMAIN ); ?></p>
...
foreach ( $quiz_modules_all as $module ) {
