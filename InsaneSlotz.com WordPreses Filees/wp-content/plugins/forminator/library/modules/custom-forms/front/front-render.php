...
//		$html .= $this->get_description( $field );
	...
	public function get_button_markup() {
        ...
        if ( 'behaviour-redirect' === $settings[ 'submission-behaviour' ] ) {
            $html .= sprintf(
                        ...
                        esc_html__( 'This form will redirect you to another page once submitted.', Forminator::DOMAIN )
                    );
        }
	...
	public function render_honeypot_field(
		...
		if ( $form_id == $this->model->id && $this->is_honeypot_enabled() ) { // WPCS: loose comparison ok
			...
			$html .= sprintf( '<label for="%1$s" class="forminator-hidden" aria-hidden="true">%2$s <input id="%1$s" type="text" name="%1$s" value="" autocomplete="off"></label>', "input_$total_fields", __( 'Please do not fill in this field.', Forminator::DOMAIN ) );
		}
	 ...
	 * @return array
	 */
	public function ajax_display( $id, $is_preview = false, $data = false, $hide = true, $last_submit_data = array(), $extra = array(), $quiz_id = 0, $render_id = 0 ) {
